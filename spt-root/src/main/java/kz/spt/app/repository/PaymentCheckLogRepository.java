...
public interface PaymentCheckLogRepository extends JpaRepository<PaymentCheckLog, Long> {
    ...
    @Query("from PaymentCheckLog pcl WHERE pcl.carStateId is not null and pcl.paymentCheckType = 'STANDARD' and pcl.plateNumber = ?1 and pcl.created >= ?2 order by pcl.id asc")
