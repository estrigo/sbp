...
public class CarOutEventStrategy extends AbstractOpenStrategy {
    ...
    public void carEvent() {
        ...
        if (!gate.isSimpleWhitelist && !leftFromThisSecondsBefore && carState != null) {
            ...
            eventLogService.sendSocketMessage(EventLogService.ArmEventType.CarEvent, EventLog.StatusType.Allow, camera.getId(), eventDto.car_number, "Выпускаем авто: Авто с гос. номером " + eventDto.car_number, "Releasing: Car with license plate " + eventDto.car_number, "Freigeben: Auto mit Kennzeichen" + eventDto.car_number);
            eventLogService.createEventLog(Gate.class.getSimpleName(), camera.getId(), properties, "Выпускаем авто: Авто с гос. номером " + eventDto.car_number, "Releasing: Car with license plate " + eventDto.car_number, "Freigeben: Auto mit Kennzeichen" + eventDto.car_number);
        }
    }
