...
public interface WhitelistRepository extends JpaRepository<Whitelist, Long> {
    ...
    @Query("from Whitelist w where w.car = ?1 and w.group is null and (w.type = 'UNLIMITED' or w.type = 'CUSTOM' or (w.type = 'PERIOD' and ?2 between w.access_start and w.access_end))")
    List<Whitelist> findValidWhiteListByCar(Cars car, Date date);

    @Query("from Whitelist w LEFT JOIN FETCH w.group g where w.car = ?1 and w.group is not null and (g.type = 'UNLIMITED' or g.type = 'CUSTOM' or (g.type = 'PERIOD' and ?2 between g.access_start and g.access_end))")
